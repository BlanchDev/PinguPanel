...
function NatTable() {
  ...
  } = useRenderIPTablesTable("nat", natRules);

  const { handleDeleteChain } = useCRUDIPTables();

  return (
    <div className='nat-table w100 maxContentH column gap50'>
      <div className='box-container noborder column gap10'>
        <div className='column gap25'>
          {natRules.policies.map((policy, index) => {
                  ...
                  <div className='table-container column gap10'>
                    <div className='row aic gap10'>
                      <h3 className='purple-title'>
                        Chains{" | "}
                        {
                          natRules.details.chains.filter((chain) =>
                            chain.parents.some(
                    ...
                    <ol className='content column gap10'>
                      {renderChains(natRules.details.chains, policyName)}
                    </ol>
                  </div>
                </div>
              </div>
      ...
      <div className='box-container noborder column gap10'>
        ...
        <div className='column gap10'>
          {natRules.customChains.map((chain) => {
      ...
      {deleteChainModal && (
        <AreYouSureModal
          ...
          description={`Are you sure you want to delete this chain? \n\n iptables -t nat -F ${deleteChainModal.chainName} \n iptables -t nat -X ${deleteChainModal.chainName}`}
          handleConfirm={async () => {
            try {
              await handleDeleteChain("nat", deleteChainModal.chainName);
